...
#include "inet/networklayer/ipv4/IPv4ECMP.h"

#include "inet/networklayer/arp/ipv4/ARPPacket_m.h"
#include "inet/networklayer/contract/IARP.h"
#include "inet/networklayer/ipv4/ICMPMessage_m.h"
#include "inet/linklayer/common/Ieee802Ctrl.h"
#include "inet/networklayer/ipv4/IPv4ECMPRoutingTable.h"
...
void IPv4ECMP::initialize(int stage)
{
    if (stage == INITSTAGE_LOCAL) {
        ...
        rt = getModuleFromPar<IPv4ECMPRoutingTable>(par("routingTableModule"), this);
...
void IPv4ECMP::handleMessage(cMessage *msg)
{
...
void IPv4ECMP::endService(cPacket *packet)
{
    if (!isUp) {
        EV_ERROR << "IPv4ECMP is down -- discarding message\n";
        delete packet;
        return;
    }
...
const InterfaceEntry *IPv4ECMP::getSourceInterfaceFrom(cPacket *packet)
{
    cGate *g = packet->getArrivalGate();
    return g ? ift->getInterfaceByNetworkLayerGateIndex(g->getIndex()) : nullptr;
}

void IPv4ECMP::handleIncomingDatagram(IPv4Datagram *datagram, const InterfaceEntry *fromIE)
{
    ASSERT(datagram);
    ASSERT(fromIE);
...
void IPv4ECMP::preroutingFinish(IPv4Datagram *datagram, const InterfaceEntry *fromIE, const InterfaceEntry *destIE, IPv4Address nextHopAddr)
{
...
void IPv4ECMP::handleIncomingARPPacket(ARPPacket *packet, const InterfaceEntry *fromIE)
{
...
void IPv4ECMP::handleIncomingICMP(ICMPMessage *packet)
{
...
void IPv4ECMP::handlePacketFromHL(cPacket *packet)
{
...
void IPv4ECMP::handlePacketFromARP(cPacket *packet)
{
...
void IPv4ECMP::datagramLocalOut(IPv4Datagram *datagram, const InterfaceEntry *destIE, IPv4Address requestedNextHopAddress)
{
 ...
 */
const InterfaceEntry *IPv4ECMP::determineOutgoingInterfaceForMulticastDatagram(IPv4Datagram *datagram, const InterfaceEntry *multicastIFOption)
{
    ...
    if (!ie) {
        IPv4Route *route = rt->findBestMatchingRoute(datagram->getSrcAddress(), datagram->getDestAddress());
        if (route)
...
void IPv4ECMP::routeUnicastPacket(IPv4Datagram *datagram, const InterfaceEntry *fromIE, const InterfaceEntry *destIE, IPv4Address requestedNextHopAddress)
{
    IPv4Address srcAddr = datagram->getSrcAddress();
    IPv4Address destAddr = datagram->getDestAddress();
    EV_INFO << "Routing " << datagram << " with source = " << srcAddr << " and destination = " << destAddr << ", ";

    IPv4Address nextHopAddr;
    // if output port was explicitly requested, use that, otherwise use IPv4ECMP routing
    if (destIE) {
        ...
        // special case ICMP reply
            ...
            const IPv4Route *re = rt->findBestMatchingRoute(srcAddr, destAddr);
            if (re && re->getInterface() == destIE)
                nextHopAddr = re->getGateway();
        }
    }
    else {
        // use IPv4ECMP routing (lookup in routing table)
        const IPv4Route *re = rt->findBestMatchingRoute(srcAddr, destAddr);
        if (re) {
...
void IPv4ECMP::routeUnicastPacketFinish(IPv4Datagram *datagram, const InterfaceEntry *fromIE, const InterfaceEntry *destIE, IPv4Address nextHopAddr)
{
...
void IPv4ECMP::routeLocalBroadcastPacket(IPv4Datagram *datagram, const InterfaceEntry *destIE)
{
...
const InterfaceEntry *IPv4ECMP::getShortestPathInterfaceToSource(IPv4Datagram *datagram)
{
    return rt->getInterfaceForDestAddr(datagram->getSrcAddress());
}

void IPv4ECMP::forwardMulticastPacket(IPv4Datagram *datagram, const InterfaceEntry *fromIE)
{
    ASSERT(fromIE);
...
void IPv4ECMP::reassembleAndDeliver(IPv4Datagram *datagram)
{
    EV_INFO << "Delivering " << datagram << " locally.\n";
...
void IPv4ECMP::reassembleAndDeliverFinish(IPv4Datagram *datagram)
{
    ...
    if (protocol == IP_PROT_ICMP) {
        ...
        send(decapsulate(datagram), "preRoutingOut");    //FIXME There is no "preRoutingOut" gate in the IPv4ECMP module.
    }
    else {
...
cPacket *IPv4ECMP::decapsulate(IPv4Datagram *datagram)
{
    ...
    // original IPv4ECMP datagram might be needed in upper layers to send back ICMP error message
...
void IPv4ECMP::fragmentPostRouting(IPv4Datagram *datagram, const InterfaceEntry *ie, IPv4Address nextHopAddr)
{
    L3Address nextHop(nextHopAddr);
...
void IPv4ECMP::fragmentAndSend(IPv4Datagram *datagram, const InterfaceEntry *ie, IPv4Address nextHopAddr)
{
...
IPv4Datagram *IPv4ECMP::encapsulate(cPacket *transportPacket, IPv4ControlInfo *controlInfo)
{
    ...
    // setting IPv4ECMP options is currently not supported

    return datagram;
}

IPv4Datagram *IPv4ECMP::createIPv4Datagram(const char *name)
{
    return new IPv4Datagram(name);
}

void IPv4ECMP::sendDatagramToOutput(IPv4Datagram *datagram, const InterfaceEntry *ie, IPv4Address nextHopAddr)
{
    {
        ...
        if (!isIeee802Lan) {
            ...
            if (nextHopMacAddr.isUnspecified()) {
                ...
                ASSERT2(pendingPackets.find(nextHopAddr) == pendingPackets.end(), "IPv4ECMP-ARP error: nextHopAddr found in ARP table, but IPv4ECMP queue for nextHopAddr not empty");
...
void IPv4ECMP::sendPacketToIeee802NIC(cPacket *packet, const InterfaceEntry *ie, const MACAddress& macAddress, int etherType)
{
...
void IPv4ECMP::sendPacketToNIC(cPacket *packet, const InterfaceEntry *ie)
{
...
void IPv4ECMP::unregisterHook(int priority, INetfilter::IHook *hook)
{
...
void IPv4ECMP::dropQueuedDatagram(const INetworkDatagram *datagram)
{
...
void IPv4ECMP::reinjectQueuedDatagram(const INetworkDatagram *datagram)
{
...
INetfilter::IHook::Result IPv4ECMP::datagramPreRoutingHook(INetworkDatagram *datagram, const InterfaceEntry *inIE, const InterfaceEntry *& outIE, L3Address& nextHopAddr)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result IPv4ECMP::datagramForwardHook(INetworkDatagram *datagram, const InterfaceEntry *inIE, const InterfaceEntry *& outIE, L3Address& nextHopAddr)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result IPv4ECMP::datagramPostRoutingHook(INetworkDatagram *datagram, const InterfaceEntry *inIE, const InterfaceEntry *& outIE, L3Address& nextHopAddr)
{
    for (auto & elem : hooks) {
...
bool IPv4ECMP::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void IPv4ECMP::flush()
{
    ...
    EV_DEBUG << "IPv4ECMP::flush(): packets in queue: " << queue.str() << endl;
    queue.clear();

    EV_DEBUG << "IPv4ECMP::flush(): pending packets:\n";
    for (auto & elem : pendingPackets) {
        EV_DEBUG << "IPv4ECMP::flush():    " << elem.first << ": " << elem.second.str() << endl;
        elem.second.clear();
    }
    pendingPackets.clear();

    EV_DEBUG << "IPv4ECMP::flush(): packets in hooks: " << queuedDatagramsForHooks.size() << endl;
...
bool IPv4ECMP::isNodeUp()
{
    NodeStatus *nodeStatus = dynamic_cast<NodeStatus *>(findContainingNode(this)->getSubmodule("status"));
    return !nodeStatus || nodeStatus->getState() == NodeStatus::UP;
}

INetfilter::IHook::Result IPv4ECMP::datagramLocalInHook(INetworkDatagram *datagram, const InterfaceEntry *inIE)
{
    for (auto & elem : hooks) {
...
INetfilter::IHook::Result IPv4ECMP::datagramLocalOutHook(INetworkDatagram *datagram, const InterfaceEntry *& outIE, L3Address& nextHopAddr)
{
    for (auto & elem : hooks) {
...
void IPv4ECMP::sendOnTransportOutGateByProtocolId(cPacket *packet, int protocolId)
{
