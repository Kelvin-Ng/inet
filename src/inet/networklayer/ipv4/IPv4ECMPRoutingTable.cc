...
void IPv4ECMPRoutingTable::initialize(int stage)
{
...
void IPv4ECMPRoutingTable::configureRouterId()
{
...
void IPv4ECMPRoutingTable::refreshDisplay() const
{
    char buf[80];
...
void IPv4ECMPRoutingTable::handleMessage(cMessage *msg)
{
    throw cRuntimeError("This module doesn't process messages");
}

void IPv4ECMPRoutingTable::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
cModule *IPv4ECMPRoutingTable::getHostModule()
{
    return findContainingNode(this);
}

void IPv4ECMPRoutingTable::deleteInterfaceRoutes(const InterfaceEntry *entry)
{
    bool changed = false;
...
void IPv4ECMPRoutingTable::invalidateCache()
{
    routingCache.clear();
...
void IPv4ECMPRoutingTable::printRoutingTable() const
{
    EV << "-- Routing table --\n";
...
void IPv4ECMPRoutingTable::printMulticastRoutingTable() const
{
...
std::vector<IPv4Address> IPv4ECMPRoutingTable::gatherAddresses() const
{
...
void IPv4ECMPRoutingTable::configureInterfaceForIPv4(InterfaceEntry *ie)
{
...
InterfaceEntry *IPv4ECMPRoutingTable::getInterfaceByAddress(const IPv4Address& addr) const
{
...
void IPv4ECMPRoutingTable::configureLoopbackForIPv4()
{
...
bool IPv4ECMPRoutingTable::isLocalAddress(const IPv4Address& dest) const
{
...
bool IPv4ECMPRoutingTable::isLocalBroadcastAddress(const IPv4Address& dest) const
{
...
InterfaceEntry *IPv4ECMPRoutingTable::findInterfaceByLocalBroadcastAddress(const IPv4Address& dest) const
{
    for (int i = 0; i < ift->getNumInterfaces(); i++) {
        InterfaceEntry *ie = ift->getInterface(i);
        if (!ie->isBroadcast())
...
bool IPv4ECMPRoutingTable::isLocalMulticastAddress(const IPv4Address& dest) const
{
...
void IPv4ECMPRoutingTable::purge()
{
    bool deleted = false;
...
IPv4Route *IPv4ECMPRoutingTable::findBestMatchingRoute(const IPv4Address& src, const IPv4Address& dest) const
{
    Enter_Method("findBestMatchingRoute(%u.%u.%u.%u, %u.%u.%u.%u)", src.getDByte(0), src.getDByte(1), src.getDByte(2), src.getDByte(3), dest.getDByte(0), dest.getDByte(1), dest.getDByte(2), dest.getDByte(3));    // note: str().c_str() too slow here

    std::string srcDestBin(8, 0);
    *(int*)srcDestBin.data() = src.getInt();
    *((int*)srcDestBin.data() + 1) = dest.getInt();

    auto it = routingCache.find(srcDestBin);
    ...
    if (bestRoutes.size() == 0) {
        ...
        //EV << "(" << src.getDByte(0) << "." << src.getDByte(1) << "." << src.getDByte(2) << "." << src.getDByte(3) << ", " << dest.getDByte(0) << "." << dest.getDByte(1) << "." << dest.getDByte(2) << "." << dest.getDByte(3) << "): hashed to " << std::hash<std::string>()(srcDestBin) << " or " << murmur3_32((const unsigned char*)srcDestBin.data(), srcDestBin.size(), 123) << ", num of EC routes = " << bestRoutes.size() << std::endl;
        //IPv4Route* bestRoute = bestRoutes[std::hash<std::string>()(srcDestBin) % bestRoutes.size()];
        IPv4Route* bestRoute = bestRoutes[murmur3_32((const unsigned char*)srcDestBin.data(), srcDestBin.size(), 123) % bestRoutes.size()];
        routingCache[srcDestBin] = bestRoute;
        return bestRoute;
    }
}

InterfaceEntry *IPv4ECMPRoutingTable::getInterfaceForDestAddr(const IPv4Address& dest) const
{
...
IPv4Address IPv4ECMPRoutingTable::getGatewayForDestAddr(const IPv4Address& dest) const
{
...
const IPv4MulticastRoute *IPv4ECMPRoutingTable::findBestMatchingMulticastRoute(const IPv4Address& origin, const IPv4Address& group) const
{
...
IPv4Route *IPv4ECMPRoutingTable::getRoute(int k) const
{
...
IPv4Route *IPv4ECMPRoutingTable::getDefaultRoute() const
{
...
bool IPv4ECMPRoutingTable::routeLessThan(const IPv4Route *a, const IPv4Route *b) const
{
...
void IPv4ECMPRoutingTable::setRouterId(IPv4Address a)
{
    routerId = a;
}

void IPv4ECMPRoutingTable::internalAddRoute(IPv4Route *entry)
{
...
void IPv4ECMPRoutingTable::addRoute(IPv4Route *entry)
{
...
IPv4Route *IPv4ECMPRoutingTable::internalRemoveRoute(IPv4Route *entry)
{
...
IPv4Route *IPv4ECMPRoutingTable::removeRoute(IPv4Route *entry)
{
...
bool IPv4ECMPRoutingTable::deleteRoute(IPv4Route *entry)    //TODO this is almost duplicate of removeRoute()
{
...
bool IPv4ECMPRoutingTable::multicastRouteLessThan(const IPv4MulticastRoute *a, const IPv4MulticastRoute *b)
{
...
void IPv4ECMPRoutingTable::internalAddMulticastRoute(IPv4MulticastRoute *entry)
{
...
void IPv4ECMPRoutingTable::addMulticastRoute(IPv4MulticastRoute *entry)
{
...
IPv4MulticastRoute *IPv4ECMPRoutingTable::internalRemoveMulticastRoute(IPv4MulticastRoute *entry)
{
...
IPv4MulticastRoute *IPv4ECMPRoutingTable::removeMulticastRoute(IPv4MulticastRoute *entry)
{
...
bool IPv4ECMPRoutingTable::deleteMulticastRoute(IPv4MulticastRoute *entry)
{
...
void IPv4ECMPRoutingTable::routeChanged(IPv4Route *entry, int fieldCode)
{
...
void IPv4ECMPRoutingTable::multicastRouteChanged(IPv4MulticastRoute *entry, int fieldCode)
{
...
void IPv4ECMPRoutingTable::updateNetmaskRoutes()
{
...
bool IPv4ECMPRoutingTable::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
IPv4Route *IPv4ECMPRoutingTable::createNewRoute()
{
